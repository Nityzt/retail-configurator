...
const ScenarioCard = ({ scenario, onEdit, onDelete }: { scenario: Scenario; onEdit: () => void; onDelete: () => void }) => (
  <div className="bg-white/5 border border-white/10 p-[3vw] sm:p-4 rounded-[2vw] sm:rounded-xl hover:bg-white/8 transition-colors">
    ...
    <div className="flex gap-[1.5vw] sm:gap-2">
      <button onClick={onEdit} className="flex-1 px-[2vw] sm:px-3 py-[1vw] sm:py-1.5 bg-white/10 text-white rounded-[1vw] sm:rounded-lg hover:bg-white/20 transition-colors text-[2.5vw] sm:text-xs font-medium">Edit</button>
      <button onClick={onDelete} className="px-[2vw] sm:px-3 py-[1vw] sm:py-1.5 text-red-400 hover:bg-red-500/10 rounded-[1vw] sm:rounded-lg transition-colors text-[2.5vw] sm:text-xs font-medium">Delete</button>
    </div>
  </div>
);
  ...
  return (
    <div className="h-full w-full bg-black flex flex-col p-[3vw] sm:p-6 lg:p-8">
      ...
      <div className="flex-1 overflow-y-auto min-h-0">
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-[2vw] sm:gap-3 max-w-4xl mx-auto">
